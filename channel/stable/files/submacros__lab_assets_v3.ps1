...
$ua = 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/151.0.0.0 Safari/537.36 UltimateMacroStrategyLab/0.2.11'
...
function Write-Status([string]$LastError='') {
    ...
    $text = "[Sync]`r`nTowers=$towerOK`r`nMaps=$mapOK`r`nMisses=$misses`r`nErrors=$errors`r`nApiFallbacks=$apiFallbacks`r`nDownloadFallbacks=$downloadFallbacks`r`nOriginalRetries=$originalRetries`r`nOptimized=$optimized`r`nLastError=$safeError`r`nCompleted=$(Get-Date -Format o)`r`n"
...
function Force-FandomOriginalUrl([string]$Url) {
    if ([string]::IsNullOrWhiteSpace($Url)) { return $Url }

    # Fandom's CDN may negotiate a WebP representation even when the MediaWiki
    # imageinfo URL ends in .png. format=original asks the CDN for the uploaded bytes.
...
function Simplify-FandomImageUrl([string]$Url) {
    $result = $Url
    # Remove thumbnail transforms if imageinfo ever hands one to us; keep revision/latest.
    $result = $result -replace '(?i)/scale-to-(?:width|height)(?:-down)?/\d+',''
    return Force-FandomOriginalUrl $result
}
...
function Optimize-Image([string]$InputPath,[string]$BasePath,[ValidateSet('tower','map')][string]$AssetType) {
    ...
    try {
        if ($AssetType -eq 'tower') {
            $maxW = 256; $maxH = 256; $quality = 86
        } else {
            $maxW = 1280; $maxH = 720; $quality = 82
        }
...
function Invoke-ImageDownload([string]$Url,[string]$Target,[bool]$CurlOnly=$false) {
    ...
    if (!$CurlOnly) {
        try {
            Invoke-WebRequest -UseBasicParsing -Uri $Url -OutFile $Target `
                -Headers (Common-Headers $accept) -TimeoutSec 45 | Out-Null
            return
        } catch {
    ...
    & curl.exe -L --fail --silent --show-error --compressed --connect-timeout 15 --max-time 60 `
        -A $ua -e "$wikiRoot/" -H "Accept: $accept" -H 'Accept-Language: en-US,en;q=0.9' `
...
function Download-Image([string]$Url,[string]$BasePath,[ValidateSet('tower','map')][string]$AssetType) {
    ...
    $originalUrl = Simplify-FandomImageUrl $Url
    try {
        Invoke-ImageDownload $originalUrl $tmp
        if (!(Test-Path -LiteralPath $tmp) -or (Get-Item -LiteralPath $tmp).Length -lt 200) {
            throw "Downloaded asset is empty: $originalUrl"
        }

        $kind = Get-ImageKind $tmp
        if ($kind -eq 'webp') {
            # Retry explicitly through curl after forcing original format and no WebP in Accept.
            $script:originalRetries++
            Log "image WEBP retry -> original :: $originalUrl"
            Invoke-ImageDownload $originalUrl $tmp $true
            $kind = Get-ImageKind $tmp
        }

        if (!$kind) { throw 'Downloaded response is not a supported image (possible HTML/CDN response).' }
        if ($kind -eq 'webp') {
            throw "Fandom still returned WebP after format=original retry: $originalUrl"
        }
...
function Get-ImageOriginalUrl([string]$FileTitle) {
    ...
    foreach ($pageObj in $json.query.pages.PSObject.Properties.Value) {
        ...
        if ($ii.Count -gt 0 -and $ii[0].url) {
            return Force-FandomOriginalUrl ([string]$ii[0].url)
        }
    }
    return $null
}
